...
class PhotoGalleryFragment : Fragment(),PhotoGalleryAdapter.CallBack {
        ...
        viewModel.fetchGalleryPhotos().observe(viewLifecycleOwner){data ->
            try {
                if (data != null && !data.isNullOrEmpty()){
                    ...
                    binding.photoGalleryRv.adapter = PhotoGalleryAdapter(data,requireContext(),this)
                }
            }catch (e: Exception){
                e.printStackTrace()
            }
    ...
    override fun photoGalleryDetails(title: String, shortTitle: String, description: String, image: String) {
